use crate::widget::{button::Button, checkbox::Checkbox, slider::Slider, label::Label};
...
impl WidgetType {
    ...
    pub fn is_focusable(&self) -> bool {
        matches!(self, WidgetType::Button(_) | WidgetType::Checkbox(_) | WidgetType::Slider(_))
    }

    pub fn value(&self) -> Option<(String, WidgetValue)> {
        match self {
            WidgetType::Checkbox(c) => Some((c.label.clone(), WidgetValue::Bool(c.checked))),
            WidgetType::Slider(s) => Some((format!("Slider({})", s.label), WidgetValue::Int(s.value))),
            _ => None,
        }
    }
}
...
impl UI {
    ...
    pub fn add(&mut self, widget: WidgetType) {
...
pub fn run_ui(mut ui: UI) -> std::io::Result<Vec<String>> {
    enable_raw_mode()?;
